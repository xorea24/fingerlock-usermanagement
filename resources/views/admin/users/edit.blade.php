    ...
    <div class="w-full max-w-4xl px-4 fade-in">
        ...
        <div class="flex items-center justify-between mb-8">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-3xl font-extrabold text-gray-900 tracking-tight">Edit Profile</h1>
                    <p class="text-sm text-gray-500 mt-1 font-medium">Update account details for <span class="text-gray-800 font-bold">{{ $user->name }}</span></p>
                </div>
            </div>
        </div>
            ...
            <div class="p-8 md:p-12">
                <form action="{{ route('users.update', $user) }}" method="POST" class="space-y-8">
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="input-field group">
                                <label class="block text-sm font-semibold text-gray-700 mb-2 ml-1 flex justify-between">
                                    New Password 
                                    <span class="text-xs text-gray-400 font-normal mt-0.5">(Leave blank to keep)</span>
                                </label>
                                <div class="relative">
                                    <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                    ...
                    <div class="pt-6 border-t border-gray-100">
                        <div class="bg-gray-50/50 rounded-2xl border border-gray-200 p-5 flex items-start gap-4">
                            <div class="mt-1">
                                <label class="relative inline-flex items-center {{ $user->id === auth()->id() ? 'opacity-60 cursor-not-allowed' : 'cursor-pointer' }}">
                            ...
                            <div>
                                ...
                                @if($user->id === auth()->id())
                                    <p class="text-xs text-amber-600 font-medium mt-2 flex items-center gap-1"><i class="fas fa-exclamation-triangle"></i> You cannot modify your own administrative status.</p>
                                @endif
                            </div>
                        </div>
                    </div>
